...
the New Article and Edit Article forms — never duplicated. Every photo field
(the cover and each additional picture) opens through the shared media
library picker — choose an existing WordPress media item (no re-upload, no
duplicate) or upload a new one — see
[SPEC-007's Media library picker](007-site-content-management.md#media-library-picker)
for the full design; it isn't re-described here since it's shared by every
image field in the admin, articles included.

**Admin menu labels (cosmetic only)**: the sidebar/dashboard call this section
"News & Events" ("All News" / "Add New"), matching the public navbar/footer
link text. Routes (`/admin/articles`, `/admin/articles/new`,
`/admin/articles/[id]/edit`), every WordPress post, and every identifier in
this codebase (`Article`, `article`, etc., throughout this document) are
unchanged — only the admin UI's own display labels differ from the code they
describe.
...
| Media library picker on the New Article photo field: choosing an existing library item appends it as a new photo slot, no upload                                                         | Manual            | Opened `/admin/articles/new`, clicked Add Photos, searched the library, picked an existing photo — confirmed it appeared as a slot marked "Existing" and set as the cover; confirmed via the WordPress REST API no new post or media item was created by the check itself — see [SPEC-007's evidence table](007-site-content-management.md#verification-and-evidence) for the picker's own query-building/permission/e2e/manual coverage | Passed (2026-09-21) |
