...
| Implementation status | Partially implemented — storage layer, admin shell/dashboard, and the Hero and School Info section editors are built and verified locally. About, Admission, Contact, Clubs, and Gallery editors, and the Articles list/edit/trash flow, are not yet built (placeholders exist so navigation never 404s) |
...
| About       | `site-about`       | `about.ts`       | section label, heading, story paragraphs (auto-balanced into two columns), mission, vision                     |
| Admission   | `site-admission`   | `admission.ts`   | programs (icon + name + level + description), requirement categories (badge + title + items), enrollment steps |
...
| About/Admission/Contact/Clubs/Gallery editors                              | —                 | Not built yet — placeholder pages only                                                                                                                                    | Pending             |
