...
| Implementation status | Implemented — storage layer, admin shell/dashboard, the Hero, School Info, About, Admission, Contact, Clubs, and Gallery section editors, and the Articles list/edit/trash flow (step 8, detailed in [SPEC-003](003-team-admin.md#article-list-edit-and-trash-implemented)) are all built, wired to the public site, and verified locally |
...
- An admin editor per section: grouped fields matching the section's field order on the
  public page, an image picker (upload a new photo, or reuse an existing WordPress media
  library item without re-uploading it — see [Media library picker](#media-library-picker)),
  and keyboard-accessible add/edit/remove/reorder for repeatable items (programs,
  requirement categories, enrollment steps, contact cards, clubs, gallery photos, footer
  program list).
...
the spec that owns the article/post content model.
...
Every image field in the admin — section images, gallery photos, article photos, and the
school logo — shares one picker dialog (`src/components/admin/media-picker.tsx`,
`MediaPickerDialog`) instead of only offering "upload a new file." Opening it (the
existing "Replace image" / "Add Photos" / "Replace" buttons, unchanged) shows two tabs:

- **Choose from library**: lists existing WordPress media, images only, newest first,
  with each item's thumbnail and alt text. Search and pagination (12 per page) are
  WordPress REST query parameters (`media_type=image`, `search`, `page`, `per_page`,
  reading `X-WP-Total`/`X-WP-TotalPages`) — never fetch-everything-then-filter. Selecting
  an item sets the field's media ID to the existing item's ID; nothing is uploaded and no
  duplicate media is created. The per-usage alt text (the field's own alt text input) is
  untouched by picking — only the library list shows the library item's own alt text, to
  help identify it.
- **Upload**: the same drag/browse flow as before, unchanged — a newly selected file
  isn't uploaded until the section/article is saved (same as every image field already
  worked).

Implementation: `src/lib/wordpress/media-library.ts` (`listMediaLibrary`, pure business
logic, no Next.js APIs) queries `GET /wp/v2/media` through `wordpressAuthedFetch` — never
from the browser, and the WordPress credentials/REST endpoint are never exposed to it.
`src/app/admin/(protected)/media-actions.ts` (`listMediaAction`) is the one
`requireAdmin()`-gated server action every field's picker calls; it isn't tied to any one
admin route since every section and article form uses it. Because a picker selection only
ever changes which media ID a field submits (never how the field is saved), no existing
section or article save action needed to change — each already skips uploading when a
file isn't present and just uses the submitted media ID.

The dialog (Radix `Dialog` + `Tabs`) traps focus and closes on <kbd>Escape</kbd> by
default; every library item is a real `<button>`, reachable and activatable by keyboard,
not a click-only div. Empty library, no search results, and a failed load each show their
own calm state (with a retry action on failure), never a blank grid or a silent error.
...
- **Overview**: Dashboard (`/admin`) — a card per section with its last-updated time and an
  Edit link, the 5 most recent news posts, and quick actions (Add News, View site).
...
- **News & Events**: All News (`/admin/articles` — search, category filter, pagination, edit,
  trash), Add New (`/admin/articles/new`, the existing SPEC-003 publish flow) — see
  [SPEC-003](003-team-admin.md#article-list-edit-and-trash-implemented) for the list/edit/trash
  implementation. Labels only — routes, WordPress posts, and every internal identifier stay
  `article`/`articles`; "News & Events" matches the public navbar/footer link text for this
  section (`src/components/public/navbar.tsx`), even though the section's own on-page heading
  reads "News & Announcements" — a pre-existing public-site inconsistency, out of scope here.
...
2. Uploads a replacement image if a new file was selected on the picker's Upload tab
   (`uploadSectionImage`, magic-byte sniffed, 10 MB limit — the same rule as SPEC-003
   article images); if an existing library item was picked instead, or nothing changed,
   the submitted media ID is used as-is and nothing is uploaded.
...
| About (text + banner image), Admission (text + background image), Contact (text), Clubs (text), Gallery (add/remove a photo) update the public page                                      | Manual                      | Edited each via `/admin`, confirmed the change live on `/` immediately after save, then restored seeded defaults (image replacements re-uploaded the same seeded source file, before the media library picker existed)                                                                                                                                                                                                              | Passed (2026-09-21) |
| Media library picker: list query building (search, pagination, images-only filter), permission gate                                                                                      | Unit + integration test     | `tests/integration/media-library.test.ts`, `tests/integration/media-actions.test.ts`                                                                                                                                                                                                                                                                                                                                                | Passed              |
| Picker dialog opens from a section editor, switches tabs, closes on Escape                                                                                                               | E2E test                    | `tests/e2e/media-picker.spec.ts`                                                                                                                                                                                                                                                                                                                                                                                                    | Passed              |
| Replace an image by choosing an existing library photo (not uploading), confirm the public page updates, confirm no new media item was created, then restore the original via the picker | Manual                      | Replaced the About banner via `/admin/sections/about`'s picker (searched the library, picked Admission.webp instead of classroom.webp), confirmed `/#about` served the newly picked image, confirmed via the WordPress REST API that the media library's item count and IDs were unchanged (no duplicate), then picked classroom.webp back and confirmed both the public page and the stored media ID were restored to the original | Passed (2026-09-21) |
| Articles list/edit/trash — schemas, list query building, edit-content round-trip detection, update/trash mutations, server actions, and the full manual create/edit/trash flow           | Unit + integration + manual | `tests/unit/article-content.test.ts`, `tests/integration/admin-articles.test.ts`, `tests/integration/edit-article.test.ts`, `tests/integration/article-management-actions.test.ts` — see [SPEC-003](003-team-admin.md#verification-and-evidence) for the full evidence table                                                                                                                                                        | Passed (2026-09-21) |
