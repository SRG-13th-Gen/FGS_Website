...
`ADMIN_DEV_EMAIL`, `ADMIN_DEV_PASSWORD`, and `ADMIN_DEV_SESSION_SECRET` support the temporary local admin login; `pnpm setup:env` seeds them in `.env.local`. Production identity and email configuration are absent until those integrations are selected. No optional application database is provisioned. WordPress connects through the `database` service hostname; Next.js uses the host-facing CMS URL. A callback from inside a container cannot use `localhost` to address host Next.js.
