...
| DEC-103 team identity                 | `next-auth@4.24.15` with Google OAuth, verified email and exact allowlist, eight-hour encrypted JWT sessions | Owner selected school Google sign-in for launch. Hosted client/allowlist configuration and MFA policy still require school setup.                                                                   |
...
| DEC-105 revalidation                  | Version-controlled WordPress must-use plugin and authenticated Next.js route; 60-second fallback             | Owner selected native CMS event refresh for launch. Hosted delivery and cache behavior remain to verify.                                                                                            |
