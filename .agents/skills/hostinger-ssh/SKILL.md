...
Before connecting, check whether SSH is enabled for this hosting account. The preview endpoint was reachable on 2026-09-27, and its presented key matched the locally saved key; authenticated access was not tested. Check SSH status under the preview website's **Advanced → SSH Access** page if connection fails. Do not interpret a connection failure as a bad password until SSH status and network reachability are checked.
...
This local file is not available to GitHub Actions. For the staging workflow, enter both values separately as GitHub `preview` environment secrets. Do not upload `.env.hostinger-ssh.local` to Hostinger or include it in deployment archives.
