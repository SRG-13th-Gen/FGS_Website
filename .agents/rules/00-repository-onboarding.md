...
`src/app` contains the public site, article detail, admin, OAuth callback, and revalidation routes; `src/components/public` and `src/components/admin` contain the custom interfaces; `src/components/ui` contains shadcn primitives. `src/lib/wordpress` holds the server-only CMS adapter, `src/lib/auth` Google session/allowlist checks, and `src/lib/env` server configuration validation. `tests/` and `scripts/` cover current workflows and local tooling. Hosted identity, inquiry delivery, and deployment remain unverified or planned. Follow the authority model in the documentation index; record significant scope/decision changes in the maintained sources.
