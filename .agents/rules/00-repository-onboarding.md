...
`src/app` contains the public site, article detail, and admin routes; `src/components/public` and `src/components/admin` contain the custom interfaces; `src/components/ui` contains shadcn primitives. `src/lib/wordpress` holds the server-only CMS adapter, `src/lib/auth` the temporary development login, and `src/lib/env` server configuration validation. `tests/` and `scripts/` cover current workflows and local tooling. Production identity, CMS event revalidation, inquiry delivery, and deployment remain planned. Follow the authority model in the documentation index; record significant scope/decision changes in the maintained sources.
