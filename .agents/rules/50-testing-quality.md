...
- Inspect actual tooling before choosing commands. `pnpm verify` runs lint, type checks, unit and mocked integration tests, formatting, and a production build. `pnpm test:e2e` builds and runs production browser smoke tests. `pnpm docker:config` validates Compose after local environment setup. These checks cover implemented behavior within their stated limits; they do not establish production acceptance.
...
- Check docs links, status consistency, traceability, and whitespace for documentation work. Preserve the existing DESIGN guidance and report CI/CD setup and hosted verification status accurately.
